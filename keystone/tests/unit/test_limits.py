...
class RegisteredLimitsTestCase(test_v3.RestfulTestCase):
    ...
    def test_create_multi_registered_limit(self):
        ...
        ref2 = unit.new_registered_limit_ref(
            service_id=self.service_id, resource_name='snapshot'
        )
        r = self.post(
            '/registered_limits',
    ...
    def test_create_registered_limit_return_count(self):
        ...
        ref2 = unit.new_registered_limit_ref(
            service_id=self.service_id2, region_id=self.region_id2
        )
    ...
    def test_update_registered_limit(self):
        ...
        self.assertEqual(
            new_registered_limits['description'], 'test description'
        )
...
class LimitsTestCase(test_v3.RestfulTestCase):
    ...
    def setUp(self):
        ...
        ref2 = unit.new_registered_limit_ref(
            service_id=self.service_id2, resource_name='snapshot'
        )
        ref3 = unit.new_registered_limit_ref(
            service_id=self.service_id,
            region_id=self.region_id,
        ...
        PROVIDERS.resource_api.create_project(
            self.project_2_id, self.project_2
        )
    ...
    def test_create_limit_with_domain_as_project(self):
        ...
        r = self.post(
            '/limits', body={'limits': [ref]}, token=self.system_admin_token
        )
        limits = r.result['limits']
        self.assertIsNone(limits[0]['project_id'])
    ...
    def test_create_limit_with_invalid_input(self):
        ref1 = unit.new_limit_ref(
            project_id=self.project_id, resource_limit='not_int'
        )
        ref2 = unit.new_limit_ref(
            project_id=self.project_id, resource_name=123
        )
        ref3 = unit.new_limit_ref(
            project_id=self.project_id, region_id='fake_region'
        )
    ...
    def test_list_limit(self):
        ...
        r = self.get(
            '/limits?resource_name=volume', expected_status=http.client.OK
        )
        limits = r.result['limits']
        self.assertEqual(len(limits), 1)
        for key in [
...
class StrictTwoLevelLimitsTestCase(LimitsTestCase):
    ...
    def test_create_multi_limits(self):
        ...
        ref_E = unit.new_limit_ref(
            project_id=self.project_E['id'],
            service_id=self.service_id,
            region_id=self.region_id,
            resource_name='volume',
            resource_limit=5,
        )
        ref_F = unit.new_limit_ref(
            project_id=self.project_F['id'],
            service_id=self.service_id,
            region_id=self.region_id,
            resource_name='volume',
            resource_limit=4,
